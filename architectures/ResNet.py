...
class ResNet(nn.Module):
    ...
    def kh_vis_init(self):
        ...
        # visualisation = []
        def hook_fn(m, i, o):
            # self._visualisation[m] = o
            # if self._b_visout:
            self._visualisation.append(o)

        for name, layer in self._modules.items():
            layer.register_forward_hook(hook_fn)
            print('submit :)', name)
        # for name, layer in enumerate(nn.ModuleList(list(self.features))):
        #     layer.register_forward_hook(hook_fn)
        print(len(self._modules.items()), len(self._visualisation))

    def get_layer_outputs(self, input):
        tmp_out = self.forward(input)
        return self._visualisation  # , tmp_out.cpu()
